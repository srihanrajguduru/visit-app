...
export async function getAreaVisitScore(latitude: number, longitude: number, areasCache?: any[]) {
    ...
    if (!areas || areas.length === 0) {
        ...
        if (!data || data.length === 0) throw new Error("No properties or areas found.");
        areas = data;
    }
    ...
    // If click is too far (> 15km strictly), we might want to reject, but for prototype we match nearest.
    ...
    if (metricsData) {
        ...
        infrastructure_score = metricsData.amenity_score || null;
        social_score = metricsData.women_safety_score || null;
